    ...
    public void action_replante() {
        for (Plantation p : plantations) {
            if (p.estMorte() && seuil_replante(p.getTypeFeve())) {
                p.Replante();
                Journalterrains.ajouter(Filiere.LA_FILIERE.getEtape() + " : Replantation de " + p.getParcelles()
                        + " parcelles de " + p.getTypeFeve());
            } else if (p.estMorte() && !seuil_replante(p.getTypeFeve())) {
                // Journalterrains.ajouter(Filiere.LA_FILIERE.getEtape()+" : Pas de replantation
                // de "+p.getTypeFeve()+" car le stock est trop important");
            } else {
            }
        }
    }
